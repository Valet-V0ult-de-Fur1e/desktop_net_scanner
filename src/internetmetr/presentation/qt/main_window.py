...
class MainWindow(QMainWindow):
    def __init__(self, view_model: MainViewModel) -> None:
        ...
        self._speed_button = QPushButton("Run speed test")
        self._trace_button = QPushButton("Open trace window")
        self._theme_button = QPushButton("Dark theme")
        ...
        actions.addWidget(self._speed_button)
        actions.addWidget(self._trace_button)
        actions.addWidget(self._theme_button)
    ...
    def _on_theme_toggled(self, checked: bool) -> None:
        self._theme_button.setText("Light theme" if checked else "Dark theme")
    ...
    def _toggle_trace_window(self) -> None:
        if self._trace_window.isVisible():
            ...
            self._trace_button.setText("Open trace window")
        else:
            self._trace_window.show()
            self._trace_button.setText("Hide trace window")
    ...
    def _advance_progress_while_running(self) -> None:
        ...
        if current < 90:
            self._progress_bar.setValue(current + 2)

    def refresh_network_data(self) -> None:
        try:
            state = self._view_model.load_network_state()
            self._hostname_card.set_value(state.hostname)
            self._local_ip_card.set_value(state.local_ip)
            self._public_ip_card.set_value(state.public_ip)
    ...
    def start_speed_test(self) -> None:
        ...
        self._status_label.setText("Running speed test (download/upload/ping)...")
        self._append_trace_line("Running new speed test")
        self._download_card.set_value("Calculating...")
        self._upload_card.set_value("Calculating...")
        self._ping_card.set_value("Calculating...")
        self._quality_card.set_value("Calculating...")
        self._progress_bar.setValue(0)
    ...
    def _on_speed_test_finished(
        ...
        self._download_card.set_value(f"{download_mbps:.2f} Mbps")
        self._upload_card.set_value(f"{upload_mbps:.2f} Mbps")
        self._ping_card.set_value(f"{ping_ms:.2f} ms")
        ...
        self._quality_card.set_value(f"{quality_score}/10 {quality_label}")
    ...
    def _on_speed_test_failed(self, error_text: str) -> None:
        ...
        self._download_card.set_value("-")
        self._upload_card.set_value("-")
        self._ping_card.set_value("-")
        self._quality_card.set_value("-")
